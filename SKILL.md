...
| 广告订单占比 | 无默认 | **需用户提供**（见规则13） | ❌ 精铺默认50%，有Amazon's Choice等强信号时可更低 |

**逻辑**：采购价和头程无法从 CLI 推导——**正向模式(forward)必须提供**，缺则做反向财务(reverse)输出总落地成本上限。**反向模式允许缺失采购/头程**，只输出"要达到 M≥2.0，总落地成本不能超过 $X"。CPC默认取市场CPC。CVR必须人工填写(Sorftime不提供点击CVR)。退货率按规则11。**⛔ 广告占比不能一律按50%默认值判生死**——产品有 Amazon's Choice/高评分/长销售历史时，询问用户实际占比。
