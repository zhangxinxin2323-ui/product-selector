...
**逻辑**：采购价和头程无法从 CLI 推导——**正向模式(forward)必须提供**，缺则做反向财务(reverse)输出总落地成本上限。**反向模式允许缺失采购/头程**，只输出"要达到 M≥2.0，总落地成本不能超过 $X"。CPC默认取市场CPC。CVR必须人工填写(Sorftime不提供点击CVR)。退货率按规则11。广告占比默认50%作为基准敏感性分析起点，**不用于单点判生死**。
